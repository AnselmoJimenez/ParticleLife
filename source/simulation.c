    ...
    while (!WindowShouldClose()) {
        ...
        *screen_height = GetScreenHeight();
        ...
        for (int i = 0; i < MAXPARTICLES; i++) {
            ...
            if (particles[i].position.x > (float) (*screen_width - DIAMETER)) {
            ...
            else if (particles[i].position.y > (float) (*screen_height - DIAMETER)) {
                particle_t temp = new_particle(
                    (Vector2) {
                ...
                if (particles[i].position.y > (float) (*screen_height + DIAMETER)) 
